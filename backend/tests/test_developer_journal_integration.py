...
def test_developer_single_consumption_atomic_save(context, outcome, monkeypatch):
    base, task = context
    ...
    with base.conn() as conn:
        ...
        try:
            ...
            proposal = dict(version=1, context_checksum=ctx['context_checksum'], summary='Synthetic design',
            ...
            trace = dict(provider=settings['ai']['provider_type'], model=settings['model_routes']['etl_specification'],
                prompt_version=PROMPT_VERSION, run_id=str(run['run_id']), input_checksum=run['input_checksum'],
                context_checksum=ctx['context_checksum'], prompt_checksum=digest(PROMPT),
                schema_checksum=digest(DeveloperProposalV1.model_json_schema()), output_checksum=digest(proposal),
