...
def stage_csv_sources(run_id, config):
    ...
    On a later source failure ExitStack cleans all earlier attempt copies, not
    ...
    with ExitStack() as stack:
        ...
        for index, source in enumerate(config['sources']):
            ref = f'source.{index}'
            item = stack.enter_context(stage_csv_source(canonical_id, source, contracts['sources'][ref]))
            staged[ref] = {**item, 'source_ref': ref}
        yield {'run_id': canonical_id, 'sources': staged, 'execution_authorized': False}
